...
def preprocess_games_data(games_df):
    ...
    # FIXED: previously missing WL was filled with 'N/A'
    before = len(games_df)
    games_df = games_df[games_df['WL'].isin(['W', 'L'])].copy()
    dropped = before - len(games_df)
    if dropped > 0:
        print(f"[preprocessing] Dropped {dropped} rows with no W/L result.")
